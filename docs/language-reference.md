...
| 语法                                   | 示例                          | 说明                                 |
| -------------------------------------- | ----------------------------- | ------------------------------------ |
| `数组 数组名[长度] = {内容}`           | `数组 示例[3] = {0}`          | 一维数组声明                         |
| `数组 数组名[行][列] = {内容}`         | `数组 矩阵[2][3] = {0}`       | 多维数组声明（逐维长度表达式）       |
| `数组 数组名[长度]`                    | `数组 示例[3]`                | 声明时省略初始化，默认全 0           |
| `数组名[索引] = 内容`                  | `示例[0] = 25`                | 替换指定索引位置处的值               |
| `数组名[索引]`                         | `示例[1]`                     | 读取指定索引位置处的值（作表达式）   |
| `数组名[行][列]`                       | `矩阵[0][1]`                  | 多维读取 / 写入（多维下标）          |

规则：

- **长度**：任意表达式；编译期常量时要求 > 0（`数组 示例[0]`、`数组 示例[1-2]` 编译期报错），动态长度运行期由 `ARRNEW` 校验。
- **初始化列表**三种形态：
  - **广播**：单个标量 `{0}` 填充全部元素（`数组 示例[3] = {7}` → `[7,7,7]`）。
  - **扁平列表**：多个标量按 row-major 平铺填充，短于长度补 0、长于长度报错（`数组 矩阵[2][2] = {1,2,3}` → `[[1,2],[3,0]]`）。
  - **嵌套列表**：`矩阵[2][3] = {{1,2,3},{4,5,6}}`，嵌套深度必须等于维度数，每层短补 0、长报错；除首维外各维须为编译期常量。
- **索引**：任意表达式，运行期求值；越界或为负抛运行时错误 `数组下标越界`。
- **多维数组**：扁平存储（row-major）。声明处下标数量必须等于维度数；经函数参数等不透明句柄访问时由运行时按实际维度解析（下标不足抛 `数组下标数量不足`）。
- **引用语义**：数组是运行时数组池的句柄，赋值/传参会共享同一份数组（与 Lua table 类似）。
- **数组类型系统**：`数组` 作为独立类型记录于符号表（`arrayRank`），标量被索引在编译期报错 `索引的目标不是数组`；rank≥2 的数组下标数量不符在编译期报错 `需要 N 个下标`。
...
| 0x19 | ARRGETN rd, rsArr, #count | 变长下标读取：从值栈弹 count 个下标后 rd = 数组[...] |
| 0x1A | ARRSETN rsVal, rsArr, #count | 变长下标写入：从值栈弹 count 个下标后 数组[...] = rsVal |
